...
MAP.set(StateActions.BOOT, (state: State) => {
  connectDb('')
  return state;
})
...
MAP.set(
  StateActions.ADD_REMOVE_DAY_OFF,
  (state: State, action: Action<Dayjs>) => {
    if (!action.payload) return state;
    const isPresent = state.daysOff.find((d) =>
      d.isSame(action.payload, 'date')
    );
    if (isPresent)
      return assoc(
        'daysOff',
...
MAP.set(
  StateActions.CREATE_ENGINEER,
  (state: State, action: Action<Engineer>) => {
    if (!action.payload) return state;
...
MAP.set(StateActions.ADD_ENGINEER, (state: State, action: Action<string>) => {
  ...
  return assoc(
    'addedEngineers',
    [...state.addedEngineers, action.payload],
    state
  );
});
...
MAP.set(
  StateActions.ASSIGN_WORK,
  (state: State, action: Action<AssignedWork>) => {
    if (!action.payload) return state;
    return assoc(
      'assignedWorks',
    ...
    return assoc(
      ...
      state.engineers.map((engineer) => {
        if (engineer.id === payload.engineerId)
          return assoc('daysOff', payload.days, engineer);
        return engineer;
      }),
      state
    );
